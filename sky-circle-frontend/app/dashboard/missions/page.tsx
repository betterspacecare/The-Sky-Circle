        ...
        <div className="py-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div>
                    <h1 className="text-4xl font-bold mb-2 flex items-center gap-3">
                        <Rocket className="w-10 h-10 text-cosmic-purple" />
                        Missions
                    </h1>
                    <p className="text-white/50">Complete objectives to earn points and exclusive badges</p>
                </div>
                <div className="flex gap-4">
                    <div className="glass-effect px-5 py-3 rounded-2xl flex items-center gap-3">
                        <Trophy className="w-6 h-6 text-cosmic-gold" />
                        <div>
                            <p className="text-[10px] uppercase font-bold text-white/40">Completed</p>
                            <p className="text-xl font-bold">{completedMissions.length}</p>
                        </div>
                    </div>
                    <div className="glass-effect px-5 py-3 rounded-2xl flex items-center gap-3">
                        <Zap className="w-6 h-6 text-cosmic-purple" />
                        <div>
                            <p className="text-[10px] uppercase font-bold text-white/40">Total Points</p>
                            <p className="text-xl font-bold">{userStats?.total_points || 0}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {loading ? (
                <div className="space-y-6">
                    {[1, 2].map(i => (
                        <div key={i} className="glass-effect h-64 rounded-3xl animate-pulse" />
                    ))}
                </div>
            ) : missions.length === 0 ? (
                <div className="glass-effect rounded-3xl p-16 text-center">
                    <Telescope className="w-16 h-16 text-cosmic-purple mx-auto mb-6 opacity-50" />
                    <h3 className="text-2xl font-bold mb-3">No Missions Available</h3>
                ...
                <div className="space-y-8">
                    ...
                    {activeMissions.length > 0 && (
                        <div>
                            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-white/70">
                    ...
                    {expiredMissions.length > 0 && (
                        <div>
                            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-white/40">
...
function MissionCard({ 
    ...
    return (
        <div className={cn(
            "glass-effect rounded-3xl overflow-hidden transition-all",
                    ...
                    <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 mb-4">
                            {completed ? (
                                ...
                                <span className="px-3 py-1 bg-white/10 text-white/40 rounded-full text-xs font-bold">
                                    Expired
                                </span>
                            ) : (
                                <span className="px-3 py-1 bg-cosmic-purple/20 text-cosmic-purple rounded-full text-xs font-bold flex items-center gap-1">
                                    <Clock className="w-3 h-3" /> {daysRemaining} days left
                                </span>
                            )}
                            <span className="px-3 py-1 bg-cosmic-gold/20 text-cosmic-gold rounded-full text-xs font-bold flex items-center gap-1">
                                <Star className="w-3 h-3" /> {mission.bonus_points} pts
                            </span>
                            {mission.reward_badge && (
                                <span className="px-3 py-1 bg-cosmic-pink/20 text-cosmic-pink rounded-full text-xs font-bold flex items-center gap-1">
                                    <Award className="w-3 h-3" /> Badge Reward
                                </span>
                            )}
                        </div>

                        <h3 className="text-2xl md:text-3xl font-bold mb-3">{mission.title}</h3>
                        <p className="text-white/60 mb-4 max-w-xl">{mission.description}</p>
                ...
                <div className="flex items-center gap-3 mt-6 pt-6 border-t border-white/5">
                    {completed && (
                        <button
                            onClick={onClaim}
                            disabled={claiming}
                            className="px-6 py-3 bg-gradient-to-r from-cosmic-gold to-yellow-500 text-black rounded-xl font-bold flex items-center gap-2 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                        >
                            {claiming ? (
                    ...
                    {!expired && (
                        <button
                            onClick={onToggle}
                            className="px-4 py-3 glass-inner rounded-xl font-medium flex items-center gap-2 hover:bg-white/10 transition-all text-sm"
                        >
            ...
            {expanded && (
                <div className="px-6 md:px-8 pb-6 md:pb-8">
                        ...
                        {mission.requirements.map((req) => {
                            ...
                            return (
                                <div
                                    key={req.id}
                                    className={cn(
                                        "glass-inner rounded-2xl p-4 flex items-center gap-4 transition-all",
                                        isCompleted 
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-white/40">{req.category}</p>
                                    </div>
                                    {isCompleted && (
                    ...
                    {!completed && (
                        <div className="mt-4 p-4 glass-inner rounded-xl flex items-center justify-between">
                            <p className="text-white/50 text-sm">
                                💡 Log observations of these objects to complete the mission
                            </p>
                            <a 
                                href="/dashboard/observations/new"
                                className="px-4 py-2 bg-cosmic-purple/20 hover:bg-cosmic-purple/30 text-cosmic-purple rounded-lg text-sm font-medium transition-all"
                            >
                                Log Observation
                            </a>
                        </div>
